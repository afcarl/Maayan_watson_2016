...
def compute_dist_delta(path_to_dir, domain_name, problem, problem_path, best_plan_costs, subset, agents_to_delete = []):
    ...
    pp, max_duration = compile_away_ma(path_to_dir + "/domain.pddl", path_to_dir + "/" + problem + ".pddl", "0", 'penalty_off', agents_to_delete)

    os.system("cp {0}/hyps_ind_{1}.dat ".format(problem_path, subset) + path_to_dir + "/hyps_ind_{0}.dat".format(subset))
    ...
    with open('{0}/hyps_ind_{1}.dat'.format(problem_path, subset)) as hypotheses:
        ...
        for i, hyp in enumerate(hypotheses):
            ...
            cmd = "python compile_away_observations_time_penalty.py k-domain.pddl " + path_to_dir + "/curr_hyp.pddl " + path_to_dir + "/obs_ind.txt {0}".format(
...
def compute_goal_posterior_probability_delta(cost_no_obs, cost_obs):
    ...
    if cost_no_obs == -1 or cost_no_obs == sys.maxint or cost_obs == -1 or cost_obs == sys.maxint:
        return 0
    prob = math.exp(-1 * (abs(math.log10(cost_no_obs) - math.log10(cost_obs)))) / (1 + math.exp(-1 * (abs(math.log10(cost_no_obs) - math.log10(cost_obs)))))

    return prob


def find_individual_plans(best_plan, set_of_agents, domain_actions, pp, plan_dict):
    ops_seq, agents, modified_pos_dict = sample_ops_from_plan_group_of_agents(set_of_agents, best_plan, 100, pp.agents, plan_dict)
    obs_seq, noisy_obs = get_operators_effects(domain_actions, ops_seq, 0, pp, len(best_plan), modified_pos_dict)

    return obs_seq, agents


def find_sub_plan(domain_name, subset, pp, plan_steps, clean_plan_dict, problem_file, path_to_dir):
...
def find_best_joint_pl(path_to_dir, problem_path, problem_name):
    domain_file = "k-domain.pddl_converted.pddl"
    template_file = "template.pddl"
    ...
    # while(1):
    #    continue
    cmd = "python compile_away_observations_time_penalty.py k-domain.pddl " + path_to_dir + "/template.pddl " + path_to_dir + "/obs.txt {0}".format(
        str(max_duration * 500))
    os.system(cmd)
    os.system(
    ...
    return clean_plan_dict, plan_steps, pp
...
def find_best_joint_plan(pp, path_to_dir, subset, domain_name, problem_file):
    ...
    print("./lpg-td -o k-domain.pddl_converted.pddl_added_goal_actions.pddl -f {0}/template.pddl_converted.pddl_added_goal_actions.pddl -nobestfirst -n 33 -cputime {1} -out plan_lpg_td > out_lpg_td.txt".format(path_to_dir, timeout))
    make_span = sys.maxint
    print("Using LPG-TD")
    plan_template = "plan_lpg_td_SOLUTION_NUM.SOL"
    ...
    individual_obs, agents_to_keep = find_individual_plans(plan_steps, agents_to_follow, domain_actions, pp, clean_plan_dict)
...
def compute_dist(path_to_dir, domain_name, problem_name, problem_path, subset, plan_steps, clean_plan_dict, pp, best_plan_costs_dict):
    problem_file = open(path_to_dir + "/" + problem_name + ".pddl", 'r')
    agents_to_delete = find_sub_plan(domain_name, subset, pp, plan_steps, clean_plan_dict, problem_file, path_to_dir)
    #agents_to_delete = find_(pp, path_to_dir, subset, domain_name, problem_file)  # also return plan goal
    if agents_to_delete == -1:
        return -1

    ret_val = delta('lpg', domain_name, problem_name, path_to_dir, problem_path, subset, best_plan_costs_dict, agents_to_delete)
...
def ind_goal_rec(domain = 0):
    ...
    res_file = open('results_IND.txt', 'w')
    res_file.close()
                        ...
                        for pct in percentages:
                            for val in noise:
                                ...
                                for subset in agent_subsets:
                                    ...
                                    working_file.write(
                                        '{0}_{1}_{2}_{3}_{4}_{5}_Working\n'.format(domain_name, problem, plan, pct, val, subset_stripped))
                                    working_file.close()

                                    start = datetime.datetime.now()
                                    ret_val = compute_dist(path, domain_name, problem, problem_path, subset_stripped, plan_steps, clean_plan_dict, pp, best_plan_costs_dict)
    #                                print("DONE WITH EVERYTHING")
                                    ...
                                    if ret_val == -1:
                                        print("**** TIMEOUT ****")
                                        timeouts.append(domain_name + " " + problem + '_{0}_{1}_{2}'.format(str(pct), str(val), subset_stripped))
                                        continue
        ...
        append_res_file('results_IND.txt'.format(subset), percentages, noise, timeouts, domain_name)


def delta(planner, domain_name, problem, path, problem_path, subset, best_plan_costs_dict, agents_to_delete = []):
    best_plan_costs = []
    compile_away_ma(problem_path + "/domain.pddl", problem_path + "/" + problem + ".pddl", "0", 'penalty_off', agents_to_delete)
    ...
    with open('{0}/hyps_ind_{1}.dat'.format(problem_path, subset), 'r') as hypotheses:
        ...
        for i, hyp in enumerate(hypotheses):
            if i > 3:
                break
            print(hyp)
            # if hash(hyp) in best_plan_costs_dict:
            #     best_plan_costs.append(best_plan_costs_dict[hash(hyp)])
            #     print(problem_path)
            #     print(best_plan_costs_dict[hash(hyp)])
            #     continue
            ...
            #best_plan_costs_dict[hash(hyp)] = best_plan_cost

    ret_val = compute_dist_delta(path, domain_name, problem, problem_path, best_plan_costs, subset, agents_to_delete)
    return ret_val
