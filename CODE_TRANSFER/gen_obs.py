...
def get_operators_effects(domain_actions, ops_seq, noise_val, pp, plan_len):
  ...
  not_observed_actions = [domain_actions[action] for action in domain_actions if action not in [op.split()[0][1:].lower() for op in ops_seq]]
  ...
  for op in ops_seq:
    ...
    params_only_types = [param.strip(')') for param in curr_action.parameters.pddl_rep()[1:].split() if param.find('?') == -1  and param.find('-') == -1]
    ...
    for eff in action_effects:
      ...
      for arg in eff.args:
        ...
        try:
        	obj_list = [obj for obj in pp.objects[params_only_types[idx].lower()] if obj.find(op.split()[idx + 1].rstrip(')')) == -1]
        except:
...
def get_plans(domain_actions):
  ...
  cmd = "./lpg-td -o k-domain.pddl -f k-problem.pddl -n 10 -cputime 70 > out_lpg_td.txt"
  os.system(cmd)
  ...
  if num_of_plans < 2:
  	os.system("rm -r plan*")
  	cmd = "./lpg-td -o k-domain.pddl -f k-problem.pddl -n 10 -cputime 130 > out_lpg_td.txt"
  	os.system(cmd)
  	for i in range(1, 10):
    ...
    try:
      ...
      for step in plan:
        ...
        if float(step[0:step.find(':')]) in list(plan_dict):
          ...
          op_name = op.split()[0][1:].lower()
          curr_action = domain_actions[op_name]
          plan_dict[float(step[0:step.find(':')]) + float(curr_action.duration)] = step   # limit to 2 places after decimal point?
        
        
      sorted_steps = collections.OrderedDict(sorted(plan_dict.items()))
      
      plan_steps = [sorted_steps[step][sorted_steps[step].find('(') : (sorted_steps[step].find('[') - 1)] for step in sorted_steps]
...
def create_PR_problems(domain_file, problem_file, path_to_prob_dir):
  ...
  # Get plans for sub problems #
	plans = get_plans(domain_actions)
