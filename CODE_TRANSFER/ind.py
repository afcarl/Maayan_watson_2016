...
def get_best_plan_cost(domain_file, problem_file, path, planner='lpg'):
    ...
    if planner == 'lpg':
        cmd = "./lpg-td -o {0} -f {2}/{1} -nobestfirst -n 20 -cputime 400 -out plan_lpg_td > out_lpg_td.txt".format(
            domain_file, problem_file, path)  # > out_lpg_td.txt -cputime 600
    if planner == 'sgplan':
        cmd = "./sgplan -o {0} -f {2}/{1} -cputime 90 -out plan_sgplan".format(domain_file, problem_file,
                                                                               path)  # > out_lpg_td.txt -cputime 600
...
def compute_goals_posterior_probability(path_to_dir, dict_of_plan_costs, set_of_plans_goals, ind = 0):
...
def compute_goal_posterior_probability_delta(cost_no_obs, cost_obs):
    print(cost_no_obs)
    print(cost_obs)
   # prob = math.exp(-1 * (abs(math.log10(cost_no_obs - cost_obs))) / (1 + math.exp(-1 * (abs(cost_no_obs - cost_obs))))
...
def find_individual_plans(best_plan, set_of_agents, domain_actions, pp):
    ops_seq, agents = sample_ops_from_plan_group_of_agents(set_of_agents, best_plan, 100, pp.agents)
    obs_seq, noisy_obs = get_operators_effects(domain_actions, ops_seq, 0, pp, len(best_plan))


    return obs_seq, agents


def find_best_joint_plan(pp, path_to_dir, d_cofficient=0.5, plan_set_size=50):
    timeout = 450
#    print("WE're HERE")
#    while(1):
#        continue
    ...
    os.system(
        "./lpg-td -o k-domain.pddl_converted.pddl_added_goal_actions.pddl -f {0}/template.pddl_converted.pddl_added_goal_actions.pddl -nobestfirst -n 20 -cputime {1} -out plan_lpg_td > out_lpg_td.txt".format(
    ...
    agent_to_follow = ["PLANE2", "PLANE3"]
    individual_obs, agents_to_keep = find_individual_plans(plan_steps, agent_to_follow, domain_actions, pp)

    # obs_file = open("obs_{0}.txt".format(str(set_of_agents)), 'w')
    obs_file = open("{0}/obs_ind.txt".format(path_to_dir), 'w')
    print("WE ARE FOLLOWING: {0}".format(agent_to_follow))
...
def compute_dist_delta(path_to_dir, domain_name, problem, problem_path, best_plan_costs, agents_to_delete = []):
    domain_file = "k-domain.pddl_converted.pddl"
    template_file = "template.pddl"
    curr_hyp_conv = "curr_hyp.pddl_converted.pddl"
    ...
    # Compile away MA - Call MA script
    pp, max_duration = compile_away_ma(path_to_dir + "/domain.pddl", path_to_dir + "/" + problem + ".pddl", "0", 'penalty_on', agents_to_delete)

    os.system("cp {0}/hyps_ind.dat ".format(problem_path) + path_to_dir + "/hyps_ind.dat")

    with open('{0}/hyps_ind.dat'.format(problem_path)) as hypotheses:
        create_template(path_to_dir)
        for i, hyp in enumerate(hypotheses):
            insert_hyp_to_template(template_file, hyp, path_to_dir)
...
def compute_dist(path_to_dir, domain_name, problem_name, problem_path, d, num_of_plans):
    ...
    os.system(cmd)
    agents_to_delete = find_best_joint_plan(pp, path_to_dir, d, num_of_plans)  # also return plan goal

    delta('lpg', agents_to_delete)
...
def diverse(d, k):
    ...
    for line in list_of_problems:
                ...
                for plan in plans_list_file:
                    for pct in percentages:
                        for val in noise:
                            ...
                            timing_file.write(
                                '{0}_{1}_{2}_{3}_{4}_{5}_{6}_Working\n'.format(domain_name, problem, plan, pct, val, d,
                                                                               k))
        ...
        append_res_file('results_DIVERSE_d:{0}_k:{1}.txt'.format(d, k), percentages, noise, timeouts)
...
def hybrid(num_of_plans):
    ...
    for line in list_of_problems:
        ...
        append_res_file('results_HYBRID.txt', percentages, noise, timeouts)


def delta(planner, agents_to_delete = []):
    percentages = [100]#[100, 70, 40, 10]
    noise = [0]  # , '14PL', '14OL']
    domains = []
    best_plan_costs = []
    ...
    for line in list_of_problems:
        ...
        for prob in line_s[1:]:
            ...
            problem_path = domain_name + "/" + problem
            # Compile away MA - Call MA script
            compile_away_ma(problem_path + "/domain.pddl", problem_path + "/" + problem + ".pddl", "0", 'penalty_on', agents_to_delete)

            with open('{0}/hyps_ind.dat'.format(problem_path), 'r') as hypotheses:
        ...
        append_res_file('results_delta.txt', percentages, noise, timeouts)
