...
def get_best_plan_cost(domain_file, problem_file, path, planner='lpg'):
  ...
  if planner == 'lpg':
    cmd = "./lpg-td -o {0} -f {2}/{1} -nobestfirst -n 10 -cputime 70 -out plan_lpg_td > out_lpg_td.txt".format(domain_file, problem_file, path) #> out_lpg_td.txt -cputime 600
  if planner == 'sgplan':
...
def compute_dist_delta(path_to_dir, domain_name, problem, problem_path, best_plan_costs):
  ...
  # Compile away MA - Call MA script
	...
	with open('{0}/hyps.dat'.format(problem_path)) as hypotheses:
		for i, hyp in enumerate(hypotheses):
			insert_hyp_to_template(template_file, hyp, path_to_dir)	
			cmd = "python compile_away_observations_time_penalty.py k-domain.pddl " + path_to_dir + "/curr_hyp.pddl " + path_to_dir + "/obs.txt {0}".format(str (max_duration * 10) )
			os.system(cmd)		
