...
def get_curr_hyp_costs_set(domain_file, problem_file, path, num_of_plans, timeout = 450):
    ...
    cmd = "./lpg-td -o {0} -f {2}/{1} -nobestfirst -n 10 -cputime {3} -out plan_lpg_td > out_lpg_td".format(domain_file, problem_file, path, timeout) #> out_lpg_td.txt -cputime 600
    os.system(cmd)
...
def get_best_plan_cost(domain_file, problem_file, path, timeout, planner='lpg'):
    os.system("rm -r *.SOL")

    if planner == 'lpg':
        cmd = "./lpg-td -o {0} -f {2}/{1} -nobestfirst -n 33 -cputime {3} -out plan_lpg_td > out_lpg_td.txt".format(domain_file, problem_file, path, timeout) #> out_lpg_td.txt -cputime 600
    if planner == 'sgplan':
...
def get_dict_of_plan_costs(path_to_dir, d_cofficient = 0.5, plan_set_size = 50, timeout = 300):

    os.system("./lpg -o k-domain.pddl_converted.pddl_added_goal_actions.pddl -f {3}/template.pddl_converted.pddl_added_goal_actions.pddl -quality -d_coefficient {0} -k_coefficient {1} -cputime {2} -out set_of_plans_lpg -v off > out_lpg.txt".format(str(d_cofficient), str(plan_set_size), timeout, path_to_dir)) # > out_lpg.txt
...
def compute_dist_hybrid(path_to_dir, domain_name, problem, problem_path, num_of_plans, include_nageted = True, timeout = 300):
...
def compute_dist_delta(path_to_dir, domain_name, problem, problem_path, best_plan_costs, include_negated = True, timeout = 300):
    domain_file = "k-domain.pddl_converted.pddl"
    template_file = "template.pddl"
    curr_hyp_conv = "curr_hyp.pddl_converted.pddl"
    ...
    with open('{0}/hyps.dat'.format(problem_path)) as hypotheses:
        for i, hyp in enumerate(hypotheses):
            ...
            if not include_negated:
                cmd = "python compile_away_observations_time_penalty.py k-domain.pddl " + path_to_dir + "/curr_hyp.pddl " + path_to_dir + "/obs_no_negated.txt {0}".format(
                    str(max_duration * 500))

            os.system(cmd)

            best_plan_cost = get_best_plan_cost(domain_file, curr_hyp_conv, path_to_dir, timeout)
    ...
    normed_goal_probs = normalize(not_normed_goal_probs)
    print(normed_goal_probs)
    create_prob_dist_file(normed_goal_probs, path_to_dir, problem_path, include_negated)
    return normed_goal_probs



def compute_dist(path_to_dir, domain_name, problem_name, problem_path, d, num_of_plans, include_nageted = True, timeout = 300):
    domain_file = "k-domain.pddl_converted.pddl"
    template_file = "template.pddl"
    ...
    os.system(cmd)
    dict_of_plan_costs, list_of_plans_goals = get_dict_of_plan_costs(path_to_dir, d, num_of_plans, timeout) # also return plan goal
...
def diverse(d, k, domain = 0):
    ...
    for line in list_of_problems:
                        ...
                        for plan in plans_list_file:
                            for pct in percentages:
                                for val in noise:
                                    ...
                                    print("***********************************************************")
                                    print(path + "      ---   d: {0} k: {1}  timeout: {2}  include_negated_obs: {3}".format(d, k, str(timeout_temp), str(truth_value)) )
                                    print("***********************************************************")
                                    ...
                                    ret_val = compute_dist(path, domain_name, problem, problem_path, d, k, truth_value, timeout_temp)
        ...
        append_res_file('results_DIVERSE_d:{0}_k:{1}.txt'.format(d, k), percentages, noise, timeouts, domain_name, timeout_list, truth_vals)

def hybrid(num_of_plans, domain = 0):
    percentages = [ 100, 70, 40, 10 ]
    noise = [0, '14PL', '14OL']
    timeout_list = [150]
    truth_vals = [True, False]
...
def delta(planner, domain = 0):
    ...
    for line in list_of_problems:
        ...
        for prob in line_s[1:]:
            ...
            with open('{0}/hyps.dat'.format(problem_path), 'r') as hypotheses:
                for hyp in hypotheses:
                    ...
                    best_plan_cost = get_best_plan_cost('k-domain.pddl', 'curr_hyp.pddl', problem_path, timeout_list[0], planner)
                        ...
                        for plan in plans_list_file:
                            for pct in percentages:
                                for val in noise:
                                    path = problem_path + "/" + plan.rstrip('\n') + "/" + str(pct) + "/" + str(val)
                                    print("*******************************")
                                    print(path + " timeout: {0}   include_negated_obs: {1}".format(timeout_temp, truth_value))
                                    print("*******************************")

                                    working_file = open('gen_goal_dist_working.txt','a')
                                    working_file.write('{0}_{1}_{2}_{3}_{4}_{5}_{6}_Working'.format(domain_name, problem, plan, pct, val, str(timeout_temp), str(truth_value)))
                                    working_file.close()

                                    start = datetime.datetime.now()
                                    ret_val = compute_dist_delta(path, domain_name, problem, problem_path, best_plan_costs, truth_value, timeout_temp)
                                    end = datetime.datetime.now()
        ...
        append_res_file('results_delta.txt', percentages, noise, timeouts, domain_name, timeout_list, truth_vals)
