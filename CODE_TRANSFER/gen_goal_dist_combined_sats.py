...
def hybrid(num_of_plans, include_negated = True, timeout = 450):
    ...
    domains = []
    timeouts = []
    list_of_problems = open('list_of_problems_sats.txt', 'r')

    dists_file = open('goal_dists_by_prob.txt','w')
    dists_file.close()

    working_file = open('gen_goal_dist_working_SATS.txt','w')
    working_file.close()

    timing_file = open('timing.txt','w')
    timing_file.close()

    res_file = open('results_HYBRID_SATS.txt','w')
    res_file.close()
                        ...
                        for plan in plans_list_file:
                            for pct in percentages:
                                for val in noise:
                                    ...
                                    working_file = open('gen_goal_dist_working_SATS.txt','a')
