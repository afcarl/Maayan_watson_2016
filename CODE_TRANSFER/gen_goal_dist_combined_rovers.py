...
def hybrid(num_of_plans, include_negated = True, timeout = 450):
    ...
    domains = []
    timeouts = []
    list_of_problems = open('list_of_problems_rovers.txt', 'r')

    dists_file = open('goal_dists_by_prob.txt','w')
    dists_file.close()

    working_file = open('gen_goal_dist_working_ROVERS.txt','w')
    working_file.close()

    timing_file = open('timing.txt','w')
    timing_file.close()

    res_file = open('results_HYBRID_ROVERS.txt','w')
    res_file.close()
    ...
    for line in list_of_problems:
                        ...
                        for plan in plans_list_file:
                            for pct in percentages:
                                for val in noise:
                                    ...
                                    working_file = open('gen_goal_dist_working_ROVERS.txt','a')
        ...
        append_res_file('results_HYBRID_ROVERS.txt', percentages, noise, timeouts, domain_name, timeout_list, truth_vals)
